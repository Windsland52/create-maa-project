...
try {
    await execFileAsync(process.execPath, ['scripts/generate-template-assets.mjs'], {
        stdio: 'inherit'
    })
    ...
    await execFileAsync(process.execPath, ['--experimental-sea-config', 'dist/sea/sea-config.json'], {
        stdio: 'inherit'
    })
    ...
    if (process.platform === 'darwin') {
        await execFileAsync('codesign', ['--sign', '-', binaryPath], { stdio: 'inherit' })
    }
