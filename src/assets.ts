...
export async function resolveOcrManifestFromEnvironment(
    ...
    return resolveProductAssetManifestFromEnvironment(
        ...
        {
            ...options,
            explicitEnvNames: ['CREATE_MAA_PROJECT_OCR_MANIFEST_URL', ...(options.explicitEnvNames ?? [])]
        }
    )
}
...
export async function downloadDefaultOcrZip(options: {
    downloader?: AssetDownloader
    url?: string
    onProgress?: DownloadProgressReporter
} = {}): Promise<DownloadedAsset[]> {
...
export function extractOcrZipAssets(zip: Buffer, sourceUrl: string): DownloadedAsset[] {
    ...
    const required = ['det.onnx', 'rec.onnx', 'keys.txt', 'README.md']
...
function mapProjectArchiveEntry(extraction: ProjectArchiveExtraction, relativePath: string): string[] {
    const mapped =
        extraction.product === 'MFAAvalonia'
...
function mapMaaFrameworkArchiveEntry(platform: string, relativePath: string): string[] {
    ...
    if (first === 'bin' && rest) {
        if (parts[1]?.toLowerCase() === 'plugins' && parts.length > 2) {
            return [`plugins/${platform}/${parts.slice(2).join('/')}`]
        }
        return [`runtimes/${platform}/native/${rest}`]
    }
    if ((first === 'lib' || first === 'plugins') && rest) {
        return [`plugins/${platform}/${rest}`]
    }
    if (first === 'share' && parts[1]?.toLowerCase() === 'maaagentbinary' && parts.length > 2) {
        return [`libs/MaaAgentBinary/${parts.slice(2).join('/')}`]
    }
    if (parts.length === 1) {
        return [`runtimes/${platform}/native/${relativePath}`]
    }
    return []
}
...
function stripArchiveRoot(
    ...
    if (product === 'MaaFramework' && ['bin', 'lib', 'share', 'include', 'plugins'].includes(root.toLowerCase())) {
        return entries
    }
    return entries
        .map((entry) => ({
...
async function fetchGithubRelease(
    ...
    const releasePath =
        channel === 'latest'
            ? 'latest'
            : `tags/${encodeURIComponent(channel)}`
...
function productManifestEnvironmentNames(product: string): string[] {
    ...
    const names = [`CREATE_MAA_PROJECT_${normalized}_MANIFEST_URL`]
