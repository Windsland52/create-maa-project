...
async function main(): Promise<void> {
    ...
    try {
        ...
        if (options.restore) {
            const restored = await withProjectWriteLock(process.cwd(), process.argv.join(' '), () =>
        ...
        if (options.acceptChangesRequested) {
            const accepted = await withProjectWriteLock(process.cwd(), process.argv.join(' '), () =>
        ...
        if (options.update.length > 0) {
            ...
            const result = await recordUpdateRequests(options, {
                productManifestResolver: (request) =>
                    resolveProductAssetManifest(request),
...
function createDownloadProgressBar(label: string, stream: NodeJS.WriteStream = process.stdout): {
...
function formatDownloadProgress(label: string, progress: DownloadProgress, columns: number): string {
    ...
    const ratio = totalBytes !== undefined && totalBytes > 0
        ? Math.min(1, progress.downloadedBytes / totalBytes)
        : undefined
    const suffix = totalBytes === undefined || ratio === undefined
