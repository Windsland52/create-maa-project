...
async function changedManagedPaths(root: string, lock: MaaProjectLock): Promise<string[]> {
    ...
    for (const [path, state] of Object.entries(lock.managedFiles).sort(([left], [right]) =>
...
export async function diffManagedFiles(root: string): Promise<string[]> {
    ...
    return lines.length > 0 ? lines : ['No managed file changes.']
}
...
): Promise<{ written: string[]; skipped: string[]; lockEntries: Record<string, ManagedFileState> }> {
...
export function createUnifiedDiff(path: string, before: string, after: string): string[] {
    ...
    return [`--- a/${path}`, `+++ b/${path}`, '@@', ...operations]
}
