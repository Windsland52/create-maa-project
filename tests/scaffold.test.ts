...
import { acceptManagedChanges, cleanCache, diffManagedFiles, restoreBackup } from '../src/project.js'
...
describe('scaffold', () => {
    ...
    it('can install node dependencies during project creation and clear node pending', async () => {
        ...
        expect(commands).toEqual([{ root: projectRoot, command: 'pnpm', args: ['install'] }])
        expect(progress).toEqual(['Installing Node dependencies...', 'Node dependencies installed.'])
        expect(result.pending.some((item) => item.kind === 'node-deps')).toBe(false)
    ...
    it('adds agent incrementally', async () => {
        ...
        const result = await addAgent(defaultOptions({ add: ['agent'] }))
    ...
    it('keeps resource packs in append order', async () => {
        ...
        await addResourcePack(defaultOptions({ add: ['resource-pack'], resourcePackSlug: 'pack-a' }))
        const result = await addResourcePack(
            defaultOptions({ add: ['resource-pack'], resourcePackSlug: 'pack-b', label: '  Pack B  ' })
        )
        ...
        expect(await readJson(join(root, 'maa-resource-test', 'interface.json'))).toMatchObject({
            resource: [
                ...
                { name: 'pack-a', path: ['./resource/pack-a'] },
    ...
    it('rejects blank resource pack labels', async () => {
        ...
        await expect(
            addResourcePack(
                defaultOptions({ add: ['resource-pack'], resourcePackSlug: 'blank-label', label: '   ' })
            )
    ...
    it('records schema-sync add-on state during project creation', async () => {
        ...
        const result = await createProject(defaultOptions({ name: 'maa-schema-sync-create', add: ['schema-sync'] }))
    ...
    it('syncs agent pyproject version metadata', async () => {
        ...
        await addAgent(defaultOptions({ add: ['agent'] }))
...
`,
    ...
    it('doctor reports path and resource config drift', async () => {
        ...
        const interfacePath = join(projectRoot, 'interface.json')
        const interfaceJson = (await readJson(interfacePath)) as Record<string, unknown>
        interfaceJson.import = ['./tasks/missing.json']
        await writeFile(interfacePath, JSON.stringify(interfaceJson, null, 4), 'utf8')
        await writeFile(
    ...
    it('generated project lint script checks project state', async () => {
        ...
        await expect(
            execFileAsync(process.execPath, ['tools/check-project.mjs'], { cwd: projectRoot })
    ...
    it('generated project lint script checks package tooling metadata', async () => {
        ...
        await expectPackageToolingError(
            (packageJson) => {
...
`,
    ...
    it('generated schema validation script checks local project JSON shape', async () => {
        ...
        await expect(
            execFileAsync(process.execPath, ['tools/validate-schema.mjs'], { cwd: projectRoot })
        ).resolves.toBeDefined()
        ...
        interfaceJson.resource = [{ name: 'base', path: './resource/base' }]
        await writeFile(interfacePath, JSON.stringify(interfaceJson, null, 4) + '\n', 'utf8')
    ...
    it('generated release dry-run smoke checks package references', async () => {
        ...
        await expect(
            execFileAsync(process.execPath, ['tools/build-release.mjs', '--dry-run'], {
                cwd: projectRoot
            })
        ...
        await expect(
            execFileAsync(process.execPath, ['tools/build-release.mjs'], {
                cwd: projectRoot,
        ...
        await writeFile(join(guiRoot, runtimePlatform.startsWith('win-') ? 'MFAAvalonia.exe' : 'MFAAvalonia'), 'gui', 'utf8')
        ...
        const interfacePath = join(projectRoot, 'interface.json')
        const interfaceJson = (await readJson(interfacePath)) as Record<string, unknown>
        interfaceJson.import = ['./README.md']
        await writeFile(interfacePath, JSON.stringify(interfaceJson, null, 4) + '\n', 'utf8')
    ...
    it('generated release staging rewrites agent child exec without changing source interface', async () => {
        ...
        expect(packageInterface.agent?.[0]?.child_args).toEqual(['python/agent/bootstrap.py'])
        expect(sourceInterface.$schema).toBeUndefined()
        expect(sourceInterface.version).toBe('v0.1.0')
    ...
    it('diffs and accepts managed local changes', async () => {
        ...
        const accepted = await acceptManagedChanges(projectRoot, ['tools/check-project.mjs'])
        const report = await runDoctor(projectRoot)

        expect(accepted).toEqual(['tools/check-project.mjs'])
        expect(report.lines.join('\n')).toContain(
    ...
    it('does not diff or accept project-owned local files as managed files', async () => {
        ...
        expect(await diffManagedFiles(projectRoot)).toEqual(['No managed file changes.'])
        await expect(acceptManagedChanges(projectRoot, ['package.json'])).rejects.toThrow(
            'Not a managed file: package.json'
        )
    })
    ...
    it('accepts all changed managed files when no paths are provided', async () => {
        ...
        expect(accepted).toEqual(['tools/check-project.mjs', 'tools/validate-schema.mjs'])
        expect(await diffManagedFiles(projectRoot)).toEqual(['No managed file changes.'])
    })
    ...
    it('backs up project state files before overwriting them', async () => {
        ...
        expect(await findBackedUpFile(join(projectRoot, '.create-maa-project/backups'), 'maa-project.lock.json')).toBe(
            true
        )
        expect(await findBackedUpFile(join(projectRoot, '.create-maa-project/backups'), 'maa-project.json')).toBe(
            true
        )
    })
    ...
    it('records remote asset update requests as pending and rejects update all', async () => {
        ...
        const result = await recordUpdateRequests(defaultOptions({ update: ['maafw', 'runtime:mfa'] }), {
        ...
        await expect(recordUpdateRequests(defaultOptions({ update: ['all'] }))).rejects.toThrow(
            '--update all is not supported'
        )
    })
    ...
    it('extracts MFAAvalonia archives into the GUI release input layout and preserves executable bits', async () => {
        ...
        const result = await recordUpdateRequests(defaultOptions({ update: ['runtime:mfa'] }), {
        ...
        expect(await readFile(join(projectRoot, '.create-maa-project/runtime/mfaa/linux-x64/MFAAvalonia'), 'utf8')).toBe('runner')
        expect((await stat(join(projectRoot, '.create-maa-project/runtime/mfaa/linux-x64/MFAAvalonia'))).mode & 0o111).not.toBe(0)
    })
    ...
    it('maps MaaFramework archive contents into agent binary and plugin directories', async () => {
        ...
        const assets = await downloadProjectManifestAssets(
            ...
            },
            {
                ...
                allowedPathPrefixes: ['.create-maa-project/runtime/', 'runtimes/', 'libs/', 'plugins/']
            }
        )
    ...
    it('downloads MFA runtime assets from a product manifest and clears runtime pending', async () => {
        ...
        const assets = new Map([
            ['https://example.test/MFAAvalonia.exe', Buffer.from('runtime')],
            ['https://example.test/MaaAgentBinary', Buffer.from('agent')],
            ['https://example.test/plugin.dll', Buffer.from('plugin')]
        ])
        const progress: string[] = []
        ...
        expect(requests).toEqual([{ product: 'MFAAvalonia', channel: 'latest' }])
        expect(result.written).toEqual(
            expect.arrayContaining([
    ...
    it('downloads OCR model assets from a verified manifest and clears OCR pending', async () => {
        ...
        const progress: string[] = []
        const result = await recordUpdateRequests(defaultOptions({ update: ['ocr-models'] }), {
            ocrManifestResolver: async () => ({
                schemaVersion: 1,
                assets: [
                    {
    ...
    it('updates embedded schema baseline and clears schema pending', async () => {
        ...
        const result = await recordUpdateRequests(defaultOptions({ update: ['schema'] }))
    ...
    it('previews schema updates without writing files', async () => {
        ...
        const preview = await previewTemplateUpdate(defaultOptions({ update: ['schema'], force: true }))
    ...
    it('runs dependency updates and clears resolved pending items', async () => {
        ...
        const result = await recordUpdateRequests(
            defaultOptions({ update: ['node-deps', 'python-deps'] }),
            {
                commandRunner: async (cwd, command, args) => {
                    commands.push({ root: cwd, command, args })
                    if (command === 'pnpm') {
        ...
        expect(commands).toEqual([
            { root: projectRoot, command: 'pnpm', args: ['install'] },
            { root: projectRoot, command: 'uv', args: ['lock'] },
            {
                root: projectRoot,
                command: 'uv',
                args: [
    ...
    it('rejects python dependency updates outside Agent projects', async () => {
        ...
        await expect(
            recordUpdateRequests(defaultOptions({ update: ['python-deps'] }), {
                commandRunner: async () => {
    ...
    it('previews template updates without writing files', async () => {
        ...
        await acceptManagedChanges(projectRoot, ['tools/check-project.mjs'])

        const preview = await previewTemplateUpdate(defaultOptions({ update: ['template'], force: true }))
    ...
    it('rejects unsupported template diff target combinations before writing', async () => {
        await expect(previewTemplateUpdate(defaultOptions({ update: ['template', 'schema'] }))).rejects.toThrow(
            '--update <target> --diff is only supported for --update template or --update schema'
        )
        await expect(previewTemplateUpdate(defaultOptions({ update: ['maafw'] }))).rejects.toThrow(
            '--update <target> --diff is only supported for --update template or --update schema'
        )
    })
    ...
    it('skips local template changes unless force is used', async () => {
        ...
        const skipped = await recordUpdateRequests(defaultOptions({ update: ['template'] }))
        ...
        const forced = await recordUpdateRequests(defaultOptions({ update: ['template'], force: true }))
    ...
    it('preserves one-time files in existing git directories', async () => {
        ...
        expect(result.skipped).toEqual(expect.arrayContaining(['README.md', 'LICENSE']))
        ...
        expect(await diffManagedFiles(target)).toEqual(['No managed file changes.'])
        ...
        expect(await diffManagedFiles(target)).toEqual(['No managed file changes.'])
        await expect(
            execFileAsync(process.execPath, ['tools/check-project.mjs'], { cwd: target })
        ).resolves.toBeDefined()
    })
    ...
    it('initializes git without committing while generated project has pending actions', async () => {
        ...
        expect(commands).toEqual([{ root: join(root, 'maa-git-pending'), args: ['init'] }])
    })
    ...
    it('allows initial commit with pending actions only when explicitly requested', async () => {
        ...
        expect(commands).toEqual([
            ['init'],
            ['add', '.'],
            ['commit', '-m', 'chore: scaffold MaaFW project']
        ])
    })
    ...
    it('cleans cache and restores backups', async () => {
        ...
        expect(restored).toEqual(['README.md'])
